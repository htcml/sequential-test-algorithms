...
def sim_peeking(muA, muB, sample_size, n_experiments, n_peeks=-1, 
                alpha=0.05, relative_mde_value=0.02, 
                tau_option=1, tau_constant=1.0, burnIn=300, 
    ...
    for n in range(start, sample_size+1, step):  
        ...
        for s in range(n_experiments):
            if msprt_res[s] == 'U':
                sa = sim_data[s][0][n-1]
                sb = sim_data[s][1][n-1]
                res = msprt_core.calc_stat(sa, n, sb, n, alpha, tau_option, tau_constant,
                                           msprt_prev_p_val[s], msprt_prev_ci[s], burnIn)
